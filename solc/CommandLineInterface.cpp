...
static bool needsHumanTargetedStdout(CommandLineOptions const& _options)
{
	...
	return _options.compiler.outputs.abi || _options.compiler.outputs.asm_ || _options.compiler.outputs.asmJson
		   || _options.compiler.outputs.yulCFGJson || _options.compiler.outputs.binary
		   || _options.compiler.outputs.binaryRuntime || _options.compiler.outputs.metadata
		   || _options.compiler.outputs.natspecUser || _options.compiler.outputs.natspecDev
		   || _options.compiler.outputs.opcodes || _options.compiler.outputs.signatureHashes
		   || _options.compiler.outputs.storageLayout || _options.compiler.outputs.transientStorageLayout;
}
...
void CommandLineInterface::handleIRAst(std::string const& _contractName)
{
	...
	if (!m_options.output.dir.empty())
		createFile(
			...
			util::jsonPrint(yulIRAst.value_or(Json{}), m_options.formatting.json));
	else
	{
		sout() << "IR AST:" << std::endl;
		sout() << util::jsonPrint(yulIRAst.value_or(Json{}), m_options.formatting.json) << std::endl;
	}
}
...
void CommandLineInterface::handleYulCFGExport(std::string const& _contractName)
{
	...
	if (!m_options.output.dir.empty())
		createFile(
			...
			util::jsonPrint(yulCFGJson.value_or(Json{}), m_options.formatting.json));
	else
	{
		sout() << "Yul Control Flow Graph:" << std::endl;
		sout() << util::jsonPrint(yulCFGJson.value_or(Json{}), m_options.formatting.json) << std::endl;
	}
}
...
void CommandLineInterface::handleIROptimized(std::string const& _contractName)
{
	...
	if (!m_options.output.dir.empty())
		createFile(m_compiler->filesystemFriendlyName(_contractName) + "_opt.yul", irOptimized.value_or(""));
	else
	{
...
void CommandLineInterface::handleIROptimizedAst(std::string const& _contractName)
{
	...
	if (!m_options.output.dir.empty())
		createFile(
			...
			util::jsonPrint(yulIROptimizedAst.value_or(Json{}), m_options.formatting.json));
	else
	{
		sout() << "Optimized IR AST:" << std::endl;
		sout() << util::jsonPrint(yulIROptimizedAst.value_or(Json{}), m_options.formatting.json) << std::endl;
	}
}

void CommandLineInterface::handleIRRocq(std::string const& _contractName)
{
	solAssert(CompilerInputModes.count(m_options.input.mode) == 1);

	if (!m_options.compiler.outputs.irRocq)
		return;

	if (!m_options.output.dir.empty())
		createFile(m_compiler->filesystemFriendlyName(_contractName) + ".v", m_compiler->yulIRRocq(_contractName));
	else
	{
		sout() << "(* Rocq *)" << std::endl;
		sout() << m_compiler->yulIRRocq(_contractName) << std::endl;
	}
}
...
void CommandLineInterface::handleTransientStorageLayout(std::string const& _contract)
{
	...
	if (!m_options.compiler.outputs.transientStorageLayout)
		return;
	std::string data
		= jsonPrint(removeNullMembers(m_compiler->transientStorageLayout(_contract)), m_options.formatting.json);
	if (!m_options.output.dir.empty())
...
std::map<std::string, Json> CommandLineInterface::parseAstFromInput()
{
	...
	for (SourceCode const& sourceCode: m_fileReader.sourceUnits() | ranges::views::values)
	{
		...
		for (auto const& [src, value]: ast["sources"].items())
		{
			...
			astAssert(
				ast["sources"][src][astKey]["nodeType"].get<std::string>() == "SourceUnit",
...
void CommandLineInterface::assembleFromEVMAssemblyJSON()
{
	...
	auto evmAssemblyStack
		= std::make_unique<evmasm::EVMAssemblyStack>(m_options.output.evmVersion, m_options.output.eofVersion);
	try
	{
...
void CommandLineInterface::compile()
{
	...
	try
	{
		...
		pipelineConfig.irOptimization = m_options.compiler.outputs.irOptimized
										|| m_options.compiler.outputs.irOptimizedAstJson
										|| m_options.compiler.outputs.irRocq || m_options.compiler.outputs.yulCFGJson;
		pipelineConfig.irCodegen
			= pipelineConfig.irOptimization || m_options.compiler.outputs.ir || m_options.compiler.outputs.irAstJson;
		pipelineConfig.bytecode = m_options.compiler.estimateGas || m_options.compiler.outputs.asm_
								  || m_options.compiler.outputs.asmJson || m_options.compiler.outputs.opcodes
								  || m_options.compiler.outputs.binary || m_options.compiler.outputs.binaryRuntime
...
void CommandLineInterface::handleAst()
{
	...
	else
	{
		...
		for (auto const& sourceCode: m_fileReader.sourceUnits())
		{
			...
			ASTJsonExporter(m_compiler->state(), m_compiler->sourceIndices())
				.print(sout(), m_compiler->ast(sourceCode.first), m_options.formatting.json);
		}
	}
}

void CommandLineInterface::handleRocq()
{
	solAssert(CompilerInputModes.count(m_options.input.mode) == 1);
	...
	if (!m_options.output.dir.empty())
	{
		for (auto const& sourceCode: m_fileReader.sourceUnits())
		{
			std::stringstream data;
			std::string postfix = "";
			ASTRocqExporter(m_compiler->state(), m_compiler->sourceIndices())
	...
	{
		sout() << "Rocq AST:" << std::endl << std::endl;
		for (auto const& sourceCode: m_fileReader.sourceUnits())
		{
			sout() << std::endl << "======= " << sourceCode.first << " =======" << std::endl;
			ASTRocqExporter(m_compiler->state(), m_compiler->sourceIndices())
...
void CommandLineInterface::assembleYul(yul::YulStack::Language _language, yul::YulStack::Machine _targetMachine)
{
		...
		if (successful && m_options.compiler.outputs.asmJson)
		{
			...
			if (result && !result->hasContiguousSourceIndices())
				solThrow(
					...
					"Source indices provided in the @use-src annotation in the Yul input do not start at 0 or are not "
					"contiguous.");
		}
	}
	...
	for (auto const& src: m_fileReader.sourceUnits())
	{
		...
		if (m_options.compiler.outputs.asmJson)
		{
			...
			sout() << util::jsonPrint(object.assembly->assemblyJSON(sourceIndices), m_options.formatting.json)
				   << std::endl;
		}
	}
}
