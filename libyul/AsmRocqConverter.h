 ...
 */
class AsmRocqConverter: public boost::static_visitor<Json>
{
public:
	...
	explicit AsmRocqConverter(Dialect const& _dialect, std::optional<size_t> _sourceIndex):
		m_dialect(_dialect),
...
private:
	...
	std::string functionName(FunctionName const& _functionName) const;
	std::string rawLiteral(Literal const& _node) const;
	template<class T>
	std::string
	rawAssign(bool isDeclaration, std::vector<T> const& variables, std::unique_ptr<Expression> const& value);
