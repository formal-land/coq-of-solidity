...
public:
	...
	void
	printSource(std::ostream& _stream, std::string const& _linePrefix = "", bool _formatted = false) const override;
	...
	std::string testRocqFilename() const;
	std::string requirePathPrefix() const;
	void writeRocqCallTest(
...
private:
	...
	TestResult tryRunTestWithYulOptimizer(std::ostream& _stream, std::string const& _linePrefix, bool _formatted);
	...
	static std::string formatEventParameter(
