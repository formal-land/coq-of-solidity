...
def node_in_block_to_rocq(definition_name: str, level: int, node) -> str:
    node_type = node.get('nodeType')

    if node_type in ['YulVariableDeclaration', 'YulAssignment']:
        return node_to_rocq(definition_name, level, node)
        ...
        return \
            ...
            indent(level + 1) + node_to_rocq(definition_name, level + 1, node) + "\n" + \
            indent(level) + "}\n" + \
...
def block_to_rocq(definition_name: str, level: int, node) -> str:
    node_type = node.get('nodeType')

    if node_type == 'YulBlock':
        statements = [
            node_in_block_to_rocq(definition_name, level, stmt)
...
def expression_to_rocq(definition_name: str) -> str:
    return f"Compare.Tactic.expression ltac:({definition_name}_deps)."

def node_to_rocq(definition_name: str, level: int, node) -> str:
    if isinstance(node, dict):
        node_type = node.get('nodeType')

        if node_type == 'YulBlock':
            return block_to_rocq(definition_name, level, node)
            ...
            return \
                ...
                indent(level) + expression_to_rocq(definition_name)
            ...
            true_body = node_to_rocq(definition_name, level, node.get('body'))
            return \
                "(* if *)\n" + \
                indent(level) + expression_to_rocq(definition_name) + "\n" + \
            ...
            cases = [
                ...
                indent(level + 1) + "Compare.Tactic.stack_primitives.\n" + \
                indent(level + 1) + node_to_rocq(definition_name, level + 1, case.get('body')) + "\n" + \
                indent(level) + "}"
...
def function_definition_to_rocq(
    contract_name: str,
    ...
    body = block_to_rocq(definition_name, level + 1, node.get('body'))
    return \
...
def block_definition_to_rocq(
    ...
    code = get_code(contract_name, is_deployed_code)
    body = block_to_rocq("body", level + 1, node)
    return \
...
def top_level_to_rocq(
    ...
    if node_type == 'YulBlock':
        ...
        for stmt in node.get('statements', []):
            if stmt.get('nodeType') == 'YulFunctionDefinition':
                lemma = function_definition_to_rocq(contract_name, module_prefix, is_deployed_code, local_functions, level, stmt)
                lemmas.append(lemma)
                local_functions.append(stmt.get('name'))
        body_lemma = block_definition_to_rocq(contract_name, module_prefix, is_deployed_code, local_functions, level, node)
...
def object_to_rocq(
    ...
    if node_type == 'YulObject':
        ...
        return \
            "Module " + name + ".\n" + \
            indent(level + 1) + object_to_rocq(contract_name, module_prefix, is_deployed_code, level + 1, node['code']) + "\n" + \
            "".join(
                "\n" +
                indent(level + 1) + object_to_rocq(contract_name, module_prefix, True, level + 1, child) + "\n"
        ...
        return top_level_to_rocq(contract_name, module_prefix, is_deployed_code, level, node['block'])
...
def main():
    ...
    rocq_code = object_to_rocq(contract_name, [], False, 0, data)
