(* Generated by rocq-of-solidity *)
Require Import RocqOfSolidity.RocqOfSolidity.

Module Test.
  Definition code : Code.t := {|
    Code.name := "Test_1595";
    Code.hex_name := 0x546573745f313539350000000000000000000000000000000000000000000000;
    Code.functions :=
      [
        
      ];
    Code.body :=
      M.scope (
        do! ltac:(M.monadic (
          M.scope (
            do! ltac:(M.monadic (
              M.declare (|
                ["_1"],
                Some (M.call_function (|
                  "memoryguard",
                  [
                    [Literal.number 0x80]
                  ]
                |))
              |)
            )) in
            do! ltac:(M.monadic (
              M.expr_stmt (|
                M.call_function (|
                  "mstore",
                  [
                    [Literal.number 64];
                    M.get_var (| "_1" |)
                  ]
                |)
              |)
            )) in
            do! ltac:(M.monadic (
              M.if_ (|
                M.call_function (|
                  "callvalue",
                  []
                |),
                M.scope (
                  do! ltac:(M.monadic (
                    M.expr_stmt (|
                      M.call_function (|
                        "revert",
                        [
                          [Literal.number 0];
                          [Literal.number 0]
                        ]
                      |)
                    |)
                  )) in
                  M.pure BlockUnit.Tt
                )
              |)
            )) in
            do! ltac:(M.monadic (
              M.declare (|
                ["_2"],
                Some (M.call_function (|
                  "datasize",
                  [
                    [Literal.string 0x546573745f313539355f6465706c6f7965640000000000000000000000000000]
                  ]
                |))
              |)
            )) in
            do! ltac:(M.monadic (
              M.expr_stmt (|
                M.call_function (|
                  "codecopy",
                  [
                    M.get_var (| "_1" |);
                    M.call_function (|
                      "dataoffset",
                      [
                        [Literal.string 0x546573745f313539355f6465706c6f7965640000000000000000000000000000]
                      ]
                    |);
                    M.get_var (| "_2" |)
                  ]
                |)
              |)
            )) in
            do! ltac:(M.monadic (
              M.expr_stmt (|
                M.call_function (|
                  "return",
                  [
                    M.get_var (| "_1" |);
                    M.get_var (| "_2" |)
                  ]
                |)
              |)
            )) in
            M.pure BlockUnit.Tt
          )
        )) in
        M.pure BlockUnit.Tt
      );
  |}.

  Module deployed.
    Definition code : Code.t := {|
      Code.name := "Test_1595_deployed";
      Code.hex_name := 0x546573745f313539355f6465706c6f7965640000000000000000000000000000;
      Code.functions :=
        [
          Code.Function.make (
            "cleanup_bool",
            ["value"],
            ["cleaned"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["cleaned"],
                  Some (M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "iszero",
                        [
                          M.get_var (| "value" |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "external_fun_verifyTx",
            [],
            [],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "callvalue",
                    []
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "slt",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.call_function (|
                            "calldatasize",
                            []
                          |);
                          M.call_function (|
                            "not",
                            [
                              [Literal.number 3]
                            ]
                          |)
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["ret"],
                  Some (M.call_function (|
                    "fun_verifyTx",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPos" |);
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "iszero",
                            [
                              M.get_var (| "ret" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "return",
                    [
                      M.get_var (| "memPos" |);
                      [Literal.number 32]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "external_fun_pair",
            [],
            [],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "callvalue",
                    []
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "slt",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.call_function (|
                            "calldatasize",
                            []
                          |);
                          M.call_function (|
                            "not",
                            [
                              [Literal.number 3]
                            ]
                          |)
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["ret"],
                  Some (M.call_function (|
                    "fun_pair",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPos" |);
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "iszero",
                            [
                              M.get_var (| "ret" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "return",
                    [
                      M.get_var (| "memPos" |);
                      [Literal.number 32]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "external_fun_testMul",
            [],
            [],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "callvalue",
                    []
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "slt",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.call_function (|
                            "calldatasize",
                            []
                          |);
                          M.call_function (|
                            "not",
                            [
                              [Literal.number 3]
                            ]
                          |)
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["zero_struct_G1Point_memory_ptr_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_struct_struct_G1Point",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "zero_struct_G1Point_memory_ptr_mpos" |);
                      [Literal.number 0x1f3aa27851409fd573e3ea94a243d632307287b4024954c113900bb4c23f3dbe]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "zero_struct_G1Point_memory_ptr_mpos" |);
                      [Literal.number 32]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "_1" |);
                      [Literal.number 0x23e15d674823d41160c3c801819a46393127b39997a9c97d4c14a0a8e4d01cf9]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["var_r_mpos"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var_r_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_struct_struct_G1Point",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 96])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 96]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.get_var (| "memPtr" |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      [Literal.number 96]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "zero_struct_G1Point_memory_ptr_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "_1" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_array_uint256",
                        [
                          M.get_var (| "memPtr" |)
                        ]
                      |);
                      [Literal.number 0x1eec35a0e955cad4bee5846ae0f1d0b742d8636b278450c534e38e06a60509f9]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_3"],
                  Some (M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "call",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.call_function (|
                                "gas",
                                []
                              |);
                              M.call_function (|
                                "not",
                                [
                                  [Literal.number 1999]
                                ]
                              |)
                            ]
                          |);
                          [Literal.number 7];
                          [Literal.number 0];
                          M.get_var (| "memPtr" |);
                          [Literal.number 0x80];
                          M.get_var (| "var_r_mpos" |);
                          [Literal.number 96]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.get_var (| "_3" |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "invalid",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_3"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr"],
                  Some (M.call_function (|
                    "eq",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "var_r_mpos" |)
                        ]
                      |);
                      [Literal.number 0x285cb7c6f3f02b845c94e350484cf47ce9f6f245c0c5609ad88aa5b86c4c3397]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.get_var (| "expr" |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["expr"],
                        Some (M.call_function (|
                          "eq",
                          [
                            M.call_function (|
                              "mload",
                              [
                                M.call_function (|
                                  "add",
                                  [
                                    M.get_var (| "var_r_mpos" |);
                                    [Literal.number 32]
                                  ]
                                |)
                              ]
                            |);
                            [Literal.number 0x0f6ca982921c98fe59a3d42894ec8e0546704a4236d52d3a87472510067351a3]
                          ]
                        |))
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPos" |);
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "iszero",
                            [
                              M.get_var (| "expr" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "return",
                    [
                      M.get_var (| "memPos" |);
                      [Literal.number 32]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "external_fun_g",
            [],
            [],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "callvalue",
                    []
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "slt",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.call_function (|
                            "calldatasize",
                            []
                          |);
                          M.call_function (|
                            "not",
                            [
                              [Literal.number 3]
                            ]
                          |)
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_mpos"],
                  Some (M.call_function (|
                    "fun_P1",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_mpos_1"],
                  Some (M.call_function (|
                    "fun_add",
                    [
                      M.get_var (| "expr_mpos" |);
                      M.call_function (|
                        "fun_negate",
                        [
                          M.call_function (|
                            "fun_P1",
                            []
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr"],
                  Some (M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "expr_mpos_1" |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.get_var (| "expr" |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["expr"],
                        Some (M.call_function (|
                          "iszero",
                          [
                            M.call_function (|
                              "mload",
                              [
                                M.call_function (|
                                  "add",
                                  [
                                    M.get_var (| "expr_mpos_1" |);
                                    [Literal.number 32]
                                  ]
                                |)
                              ]
                            |)
                          ]
                        |))
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPos" |);
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "iszero",
                            [
                              M.get_var (| "expr" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "return",
                    [
                      M.get_var (| "memPos" |);
                      [Literal.number 32]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "panic_error_0x41",
            [],
            [],
            M.scope (
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 0];
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 224];
                          [Literal.number 0x4e487b71]
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 4];
                      [Literal.number 0x41]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "revert",
                    [
                      [Literal.number 0];
                      [Literal.number 0x24]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_memory_3951",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "mload",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["newFreePtr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "memPtr" |);
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "or",
                    [
                      M.call_function (|
                        "gt",
                        [
                          M.get_var (| "newFreePtr" |);
                          [Literal.number 0xffffffffffffffff]
                        ]
                      |);
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "newFreePtr" |);
                          M.get_var (| "memPtr" |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 64];
                      M.get_var (| "newFreePtr" |)
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_memory_3958",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "mload",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["newFreePtr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "memPtr" |);
                      [Literal.number 256]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "or",
                    [
                      M.call_function (|
                        "gt",
                        [
                          M.get_var (| "newFreePtr" |);
                          [Literal.number 0xffffffffffffffff]
                        ]
                      |);
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "newFreePtr" |);
                          M.get_var (| "memPtr" |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 64];
                      M.get_var (| "newFreePtr" |)
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_memory",
            ["size"],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "mload",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["newFreePtr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "memPtr" |);
                      M.call_function (|
                        "and",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.get_var (| "size" |);
                              [Literal.number 31]
                            ]
                          |);
                          M.call_function (|
                            "not",
                            [
                              [Literal.number 31]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "or",
                    [
                      M.call_function (|
                        "gt",
                        [
                          M.get_var (| "newFreePtr" |);
                          [Literal.number 0xffffffffffffffff]
                        ]
                      |);
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "newFreePtr" |);
                          M.get_var (| "memPtr" |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 64];
                      M.get_var (| "newFreePtr" |)
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_struct_struct_G1Point",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "array_allocation_size_array_uint256_dyn",
            ["length"],
            ["size"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      M.get_var (| "length" |);
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 5];
                          M.get_var (| "length" |)
                        ]
                      |);
                      [Literal.number 0x20]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_array_array_uint256_dyn",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 0x09];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 5];
                          [Literal.number 0x09]
                        ]
                      |);
                      [Literal.number 0x20]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 0x09]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.get_var (| "memPtr_1" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 0x09];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size_1"],
                  Some (M.get_var (| "size" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "size" |);
                          M.call_function (|
                            "not",
                            [
                              [Literal.number 31]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_array_array_uint256_dyn_memory_ptr",
            ["length"],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.call_function (|
                        "array_allocation_size_array_uint256_dyn",
                        [
                          M.get_var (| "length" |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.get_var (| "length" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.get_var (| "memPtr_1" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      M.call_function (|
                        "add",
                        [
                          M.call_function (|
                            "array_allocation_size_array_uint256_dyn",
                            [
                              M.get_var (| "length" |)
                            ]
                          |);
                          M.call_function (|
                            "not",
                            [
                              [Literal.number 31]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_array_array_uint256",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 32])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 32]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size_1"],
                  Some ([Literal.number 32])
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.get_var (| "memPtr" |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      [Literal.number 32]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_struct_struct_G2Point",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 64])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      [Literal.number 64]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      M.get_var (| "memPtr_1" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_2"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.get_var (| "memPtr_2" |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      [Literal.number 64]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "memPtr_2" |)
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_struct_struct_Proof",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory_3958",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      M.get_var (| "memPtr_1" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr_2"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_2" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_2" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "memPtr_2" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 64]
                        ]
                      |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G2Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_3"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr_3"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_3" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_3" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 96]
                        ]
                      |);
                      M.get_var (| "memPtr_3" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 128]
                        ]
                      |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G1Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 160]
                        ]
                      |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G1Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 192]
                        ]
                      |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G1Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 224]
                        ]
                      |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G1Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "panic_error_0x32",
            [],
            [],
            M.scope (
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 0];
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 224];
                          [Literal.number 0x4e487b71]
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 4];
                      [Literal.number 0x32]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "revert",
                    [
                      [Literal.number 0];
                      [Literal.number 0x24]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "baseRef" |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 32]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_t_uint256_dyn_memory_ptr",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 0x01];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_uint256_dyn_3972",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 2];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 96]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_uint256_dyn_3973",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 0x03];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 128]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_array_uint256_dyn_memory_ptr",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 0x04];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 160]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_uint256_dyn_memory_ptr",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 0x05];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 192]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_t_array_t_uint256_dyn",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 0x06];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 224]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_t_uint256_dyn",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 0x07];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 256]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_array_uint256_dyn",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 0x08];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 288]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_uint256_dyn_4089",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 0x09];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 320]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_uint256_dyn",
            ["baseRef"; "index"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "index" |);
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "baseRef" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "baseRef" |);
                          M.call_function (|
                            "shl",
                            [
                              [Literal.number 5];
                              M.get_var (| "index" |)
                            ]
                          |)
                        ]
                      |);
                      [Literal.number 32]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "abi_encode_stringliteral_25aa",
            ["headStart"],
            ["tail"],
            M.scope (
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "headStart" |);
                      [Literal.number 32]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "headStart" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 22]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "headStart" |);
                          [Literal.number 64]
                        ]
                      |);
                      [Literal.string 0x5375636365737366756c6c792076657269666965642e00000000000000000000]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["tail"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "headStart" |);
                      [Literal.number 96]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_verifyTx",
            [],
            ["var_"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["var_"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1432_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_array_array_uint256_dyn",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["zero_struct_Proof_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_struct_struct_Proof",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1445_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1445_mpos" |);
                      [Literal.number 0x1c76476f4def4bb94541d57ebba1193381ffa7aa76ada664dd31c16024c43f59]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1445_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x3034dd2920f673e204fee2811c678745fc819b55d3e9d294e45c9b03a76aef41]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "zero_struct_Proof_mpos" |);
                      M.get_var (| "expr_1445_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1455_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1455_mpos" |);
                      [Literal.number 0x111e129f1cf1097710d41c4ac70fcdfa5ba2023c6ff1cbeac322de49d1b6df7c]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1455_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x103188585e2364128fe25c70558f1560f4f9350baf3959e603cc91486e110936]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "zero_struct_Proof_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_1455_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 64])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1465_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1465_mpos" |);
                      [Literal.number 0x1213d2149b006137fcfb23036606f848d638d576a120ca981b5b1a5f9300b3ee]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1465_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x2276cf730cf493cd95d64677bbb75fc42db72513a4c1e387b476d056f80aa75f]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1468_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1468_mpos" |);
                      [Literal.number 0x21ee6226d31426322afcda621464d0611d226783262e21bb3bc86b537e986237]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1468_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x096df1f82dff337dd5972e32a8ad43e28a78a96a823ef1cd4debe12b6552ea5f]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1469_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1469_mpos" |);
                      M.get_var (| "expr_1465_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1469_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_1468_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "zero_struct_Proof_mpos" |);
                          [Literal.number 64]
                        ]
                      |);
                      M.get_var (| "expr_1469_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1479_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1479_mpos" |);
                      [Literal.number 0x06967a1237ebfeca9aaae0d6d0bab8e28c198c5a339ef8a2407e31cdac516db9]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1479_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x0e4e3ed0898ba2ce904a02b739caf9931f9b43c600154592cee5239435f91f76]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "zero_struct_Proof_mpos" |);
                          [Literal.number 96]
                        ]
                      |);
                      M.get_var (| "expr_1479_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1489_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1489_mpos" |);
                      [Literal.number 0x0f25929bcb43d5a57391564615c9e70a992b10eafa4db109709649cf48c50dd2]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1489_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x16da2f5cb6be7a0aa72c440c53c9bbdfec6c36c7d515536431b3a865468acbba]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "zero_struct_Proof_mpos" |);
                          [Literal.number 128]
                        ]
                      |);
                      M.get_var (| "expr_1489_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1499_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1499_mpos" |);
                      [Literal.number 0x1b9e027bd5cfc2cb5db82d4dc9677ac795ec500ecd47deee3b5da006d6d049b8]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1499_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x1e8cfd56691c1245342d15eff3d1ce17c85fed7538c3614d46996d8f85a5c226]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "zero_struct_Proof_mpos" |);
                          [Literal.number 160]
                        ]
                      |);
                      M.get_var (| "expr_1499_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1509_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1509_mpos" |);
                      [Literal.number 0x1abb4a25eb9379ae96c84fff9f0540abcfc0a0d11aeda02d4f37e4baf74cb0c1]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1509_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x1ff09a73b455eca26257dc97dbe2c2572e162a9eefc4ce83ac7869b267acc4de]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "zero_struct_Proof_mpos" |);
                          [Literal.number 224]
                        ]
                      |);
                      M.get_var (| "expr_1509_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1519_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1519_mpos" |);
                      [Literal.number 0x2f2ea0b3da1e8ef11914acf8b2e1b32d99df51f5f4f206fc6b947eae860eddb6]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1519_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x068134ddb33dc888ef446b648d72338684d678d2eb2371c61a50734d78da4b72]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "zero_struct_Proof_mpos" |);
                          [Literal.number 192]
                        ]
                      |);
                      M.get_var (| "expr_1519_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_1432_mpos" |)
                        ]
                      |);
                      [Literal.number 0x1eec35a0e955cad4bee5846ae0f1d0b742d8636b278450c534e38e06a60509f9]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_1432_mpos" |)
                        ]
                      |);
                      [Literal.number 0x016086cc934d5cab679c6991a4efcedbab26d7e4fb23b6a1ad4e6b5c2fb59ce5]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3972",
                        [
                          M.get_var (| "expr_1432_mpos" |)
                        ]
                      |);
                      [Literal.number 0x1887420878c0c8e37605291c626585eabbec8d8b97a848fe8d58a37b00458351]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3973",
                        [
                          M.get_var (| "expr_1432_mpos" |)
                        ]
                      |);
                      [Literal.number 0x0ea71d0abb524cac7cfff5323e1d0b14ab705842426c978f96753ccce258ed93]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_array_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_1432_mpos" |)
                        ]
                      |);
                      [Literal.number 0x0689c3dc4311426ee11707866b2cbdf9751dacd07245bf99d2113d3f5a8cac47]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_1432_mpos" |)
                        ]
                      |);
                      [Literal.number 0x030f7b0c405c888aff922307ea2cd1c70f64664bab76899500341f4260a20929]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn",
                        [
                          M.get_var (| "expr_1432_mpos" |)
                        ]
                      |);
                      [Literal.number 0x0c692b41f1acc961f6ea83bae2c3a1a55c54f766c63ba76989f52c149c17b5e7]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn",
                        [
                          M.get_var (| "expr_1432_mpos" |)
                        ]
                      |);
                      [Literal.number 0x1551dcd4965285ef049512d2d30cbfc1a91acd5baad4a6e19e22e93176197f17]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_array_uint256_dyn",
                        [
                          M.get_var (| "expr_1432_mpos" |)
                        ]
                      |);
                      [Literal.number 0xe40800000000000000008cdcbc]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.switch (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "fun_verify",
                        [
                          M.get_var (| "expr_1432_mpos" |);
                          M.get_var (| "zero_struct_Proof_mpos" |)
                        ]
                      |)
                    ]
                  |),
                  [
                    (
                      Some (Literal.number 0),
                      M.scope (
                        do! ltac:(M.monadic (
                          M.assign (|
                            ["var_"],
                            Some ([Literal.number 0])
                          |)
                        )) in
                        do! ltac:(M.monadic (
                          M.leave (||)
                        )) in
                        M.pure BlockUnit.Tt
                      )
                    );
                    (
                      None,
                      M.scope (
                        do! ltac:(M.monadic (
                          M.declare (|
                            ["_2"],
                            Some (M.call_function (|
                              "mload",
                              [
                                [Literal.number 64]
                              ]
                            |))
                          |)
                        )) in
                        do! ltac:(M.monadic (
                          M.expr_stmt (|
                            M.call_function (|
                              "log1",
                              [
                                M.get_var (| "_2" |);
                                M.call_function (|
                                  "sub",
                                  [
                                    M.call_function (|
                                      "abi_encode_stringliteral_25aa",
                                      [
                                        M.get_var (| "_2" |)
                                      ]
                                    |);
                                    M.get_var (| "_2" |)
                                  ]
                                |);
                                [Literal.number 0x3f3cfdb26fb5f9f1786ab4f1a1f9cd4c0b5e726cbdfc26e495261731aad44e39]
                              ]
                            |)
                          |)
                        )) in
                        do! ltac:(M.monadic (
                          M.assign (|
                            ["var_"],
                            Some ([Literal.number 0x01])
                          |)
                        )) in
                        do! ltac:(M.monadic (
                          M.leave (||)
                        )) in
                        M.pure BlockUnit.Tt
                      )
                    )
                  ]
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_array_array_struct_G1Point_dyn",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 2];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 5];
                          [Literal.number 2]
                        ]
                      |);
                      [Literal.number 0x20]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 2]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.get_var (| "memPtr_1" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 2];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size_1"],
                  Some (M.get_var (| "size" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "size" |);
                      M.call_function (|
                        "not",
                        [
                          [Literal.number 31]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["i"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                do!
                  M.scope (
                    M.pure BlockUnit.Tt
                  ) in
                ltac:(M.monadic (
                  M.for_ (|
                    ltac:(M.monadic (
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "i" |);
                          M.get_var (| "_1" |)
                        ]
                      |)
                    )),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["i"],
                          Some (M.call_function (|
                            "add",
                            [
                              M.get_var (| "i" |);
                              [Literal.number 32]
                            ]
                          |))
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    ),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["memPtr_2"],
                          Some ([Literal.number 0])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["memPtr_2"],
                          Some (M.call_function (|
                            "allocate_memory_3951",
                            []
                          |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.get_var (| "memPtr_2" |);
                              [Literal.number 0]
                            ]
                          |)
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.get_var (| "memPtr_2" |);
                                  [Literal.number 32]
                                ]
                              |);
                              [Literal.number 0]
                            ]
                          |)
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.call_function (|
                                    "add",
                                    [
                                      M.get_var (| "memPtr_1" |);
                                      M.get_var (| "i" |)
                                    ]
                                  |);
                                  [Literal.number 32]
                                ]
                              |);
                              M.get_var (| "memPtr_2" |)
                            ]
                          |)
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    )
                  |)
                ))
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_array_array_struct_G1Point_memory_ptr_dyn_memory_ptr",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 0x0a];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 5];
                          [Literal.number 0x0a]
                        ]
                      |);
                      [Literal.number 0x20]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 0x0a]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.get_var (| "memPtr_1" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 0x0a];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size_1"],
                  Some (M.get_var (| "size" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "size" |);
                      M.call_function (|
                        "not",
                        [
                          [Literal.number 31]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["i"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                do!
                  M.scope (
                    M.pure BlockUnit.Tt
                  ) in
                ltac:(M.monadic (
                  M.for_ (|
                    ltac:(M.monadic (
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "i" |);
                          M.get_var (| "_1" |)
                        ]
                      |)
                    )),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["i"],
                          Some (M.call_function (|
                            "add",
                            [
                              M.get_var (| "i" |);
                              [Literal.number 32]
                            ]
                          |))
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    ),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["memPtr_2"],
                          Some ([Literal.number 0])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["memPtr_2"],
                          Some (M.call_function (|
                            "allocate_memory_3951",
                            []
                          |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.get_var (| "memPtr_2" |);
                              [Literal.number 0]
                            ]
                          |)
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.get_var (| "memPtr_2" |);
                                  [Literal.number 32]
                                ]
                              |);
                              [Literal.number 0]
                            ]
                          |)
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.call_function (|
                                    "add",
                                    [
                                      M.get_var (| "memPtr_1" |);
                                      M.get_var (| "i" |)
                                    ]
                                  |);
                                  [Literal.number 32]
                                ]
                              |);
                              M.get_var (| "memPtr_2" |)
                            ]
                          |)
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    )
                  |)
                ))
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_array_array_struct_G2Point_dyn",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 2];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 5];
                          [Literal.number 2]
                        ]
                      |);
                      [Literal.number 0x20]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 2]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.get_var (| "memPtr_1" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 2];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size_1"],
                  Some (M.get_var (| "size" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "size" |);
                      M.call_function (|
                        "not",
                        [
                          [Literal.number 31]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["i"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                do!
                  M.scope (
                    M.pure BlockUnit.Tt
                  ) in
                ltac:(M.monadic (
                  M.for_ (|
                    ltac:(M.monadic (
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "i" |);
                          M.get_var (| "_1" |)
                        ]
                      |)
                    )),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["i"],
                          Some (M.call_function (|
                            "add",
                            [
                              M.get_var (| "i" |);
                              [Literal.number 32]
                            ]
                          |))
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    ),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.call_function (|
                                    "add",
                                    [
                                      M.get_var (| "memPtr_1" |);
                                      M.get_var (| "i" |)
                                    ]
                                  |);
                                  [Literal.number 32]
                                ]
                              |);
                              M.call_function (|
                                "allocate_and_zero_memory_struct_struct_G2Point",
                                []
                              |)
                            ]
                          |)
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    )
                  |)
                ))
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_array_array_struct_G2Point_memory_ptr_dyn_memory_ptr",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 0x03];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 5];
                          [Literal.number 0x03]
                        ]
                      |);
                      [Literal.number 0x20]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 0x03]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.get_var (| "memPtr_1" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      [Literal.number 0x03];
                      [Literal.number 0xffffffffffffffff]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x41",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size_1"],
                  Some (M.get_var (| "size" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "size" |);
                      M.call_function (|
                        "not",
                        [
                          [Literal.number 31]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["i"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                do!
                  M.scope (
                    M.pure BlockUnit.Tt
                  ) in
                ltac:(M.monadic (
                  M.for_ (|
                    ltac:(M.monadic (
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "i" |);
                          M.get_var (| "_1" |)
                        ]
                      |)
                    )),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["i"],
                          Some (M.call_function (|
                            "add",
                            [
                              M.get_var (| "i" |);
                              [Literal.number 32]
                            ]
                          |))
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    ),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.call_function (|
                                    "add",
                                    [
                                      M.get_var (| "memPtr_1" |);
                                      M.get_var (| "i" |)
                                    ]
                                  |);
                                  [Literal.number 32]
                                ]
                              |);
                              M.call_function (|
                                "allocate_and_zero_memory_struct_struct_G2Point",
                                []
                              |)
                            ]
                          |)
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    )
                  |)
                ))
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "panic_error_0x11",
            [],
            [],
            M.scope (
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 0];
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 224];
                          [Literal.number 0x4e487b71]
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 4];
                      [Literal.number 0x11]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "revert",
                    [
                      [Literal.number 0];
                      [Literal.number 0x24]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "checked_sub_uint256",
            ["y"],
            ["diff"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["diff"],
                  Some (M.call_function (|
                    "sub",
                    [
                      [Literal.number 0x30644e72e131a029b85045b68181585d97816a916871ca8d3c208c16d87cfd47];
                      M.get_var (| "y" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      M.get_var (| "diff" |);
                      [Literal.number 0x30644e72e131a029b85045b68181585d97816a916871ca8d3c208c16d87cfd47]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x11",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_pair",
            [],
            ["var"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["var"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 64])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_845_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_845_mpos" |);
                      [Literal.number 0x0a09ccf561b55fd99d1c1208dee1162457b57ac5af3759d50671e510e428b2a1]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_845_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x2e539c423b302d13f4e5773c603948eaf5db5df8ae8a9a9113708390a06410d8]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_848_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_848_mpos" |);
                      [Literal.number 0x19b763513924a736e4eebd0d78c91c1bc1d657fee4214057d21414011cfcc763]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_848_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x2f8d9f9ab83727c77a2fec063cb7b6e5eb23044ccf535ad49d46d394fb6f6bf6]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_849_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_849_mpos" |);
                      M.get_var (| "expr_845_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_849_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_848_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_866_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_array_array_struct_G1Point_dyn",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_879_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_array_array_struct_G2Point_dyn",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_866_mpos" |)
                        ]
                      |);
                      M.call_function (|
                        "fun_mul",
                        [
                          M.call_function (|
                            "fun_P1",
                            []
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_866_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_866_mpos" |)
                        ]
                      |);
                      M.call_function (|
                        "fun_negate",
                        [
                          M.call_function (|
                            "fun_P1",
                            []
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_866_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_879_mpos" |)
                        ]
                      |);
                      M.call_function (|
                        "fun_P2",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_879_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_879_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_849_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_879_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "cleanup_bool",
                    [
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "fun_pairing",
                            [
                              M.get_var (| "expr_866_mpos" |);
                              M.get_var (| "expr_879_mpos" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["var"],
                        Some ([Literal.number 0])
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.leave (||)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_866_mpos" |)
                        ]
                      |);
                      M.call_function (|
                        "fun_P1",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_866_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_866_mpos" |)
                        ]
                      |);
                      M.call_function (|
                        "fun_P1",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_866_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.call_function (|
                            "mload",
                            [
                              M.call_function (|
                                "memory_array_index_access_t_uint256_dyn_memory_ptr",
                                [
                                  M.get_var (| "expr_866_mpos" |)
                                ]
                              |)
                            ]
                          |);
                          [Literal.number 32]
                        ]
                      |);
                      M.call_function (|
                        "checked_sub_uint256",
                        [
                          M.call_function (|
                            "mload",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.call_function (|
                                    "mload",
                                    [
                                      M.call_function (|
                                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                                        [
                                          M.get_var (| "expr_866_mpos" |)
                                        ]
                                      |)
                                    ]
                                  |);
                                  [Literal.number 32]
                                ]
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_879_mpos" |)
                        ]
                      |);
                      M.call_function (|
                        "fun_P2",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_879_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_879_mpos" |)
                        ]
                      |);
                      M.call_function (|
                        "fun_P2",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_879_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "cleanup_bool",
                    [
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "fun_pairing",
                            [
                              M.get_var (| "expr_866_mpos" |);
                              M.get_var (| "expr_879_mpos" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["var"],
                        Some ([Literal.number 0])
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.leave (||)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var"],
                  Some ([Literal.number 0x01])
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "require_helper",
            ["condition"],
            [],
            M.scope (
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.get_var (| "condition" |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_add",
            ["var_p1_90_mpos"; "var_p2_93_mpos"],
            ["var_r_mpos"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var_r_mpos"],
                  Some (M.get_var (| "memPtr" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 128])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 128]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      [Literal.number 128]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "var_p1_90_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_2"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "var_p1_90_mpos" |);
                          [Literal.number 32]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["addr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_3"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_3"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 32]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "addr" |);
                      M.get_var (| "_2" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_4"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "var_p2_93_mpos" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["addr_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_5"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_5"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr_1"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "addr_1" |);
                      M.get_var (| "_4" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_6"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "var_p2_93_mpos" |);
                          [Literal.number 32]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["addr_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_7"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_7"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr_2"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 96]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "addr_2" |);
                      M.get_var (| "_6" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["var_success"],
                  Some (M.call_function (|
                    "call",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.call_function (|
                            "gas",
                            []
                          |);
                          M.call_function (|
                            "not",
                            [
                              [Literal.number 1999]
                            ]
                          |)
                        ]
                      |);
                      [Literal.number 6];
                      [Literal.number 0];
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 0xc0];
                      M.get_var (| "memPtr" |);
                      [Literal.number 0x60]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.get_var (| "var_success" |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "invalid",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.get_var (| "var_success" |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "revert",
                          [
                            [Literal.number 0];
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_array_uint256",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_mul_3948",
            ["var_p_mpos"],
            ["var_r_mpos"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var_r_mpos"],
                  Some (M.get_var (| "memPtr" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 96])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 96]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      [Literal.number 96]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "var_p_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.call_function (|
                        "mload",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.get_var (| "var_p_mpos" |);
                              [Literal.number 32]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["addr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "addr" |);
                      [Literal.number 0x02]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_3"],
                  Some (M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "call",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.call_function (|
                                "gas",
                                []
                              |);
                              M.call_function (|
                                "not",
                                [
                                  [Literal.number 1999]
                                ]
                              |)
                            ]
                          |);
                          [Literal.number 7];
                          [Literal.number 0];
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 0x80];
                          M.get_var (| "memPtr" |);
                          [Literal.number 96]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.get_var (| "_3" |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "invalid",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_3"],
                  Some ([Literal.number 0])
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_mul",
            ["var_p_mpos"],
            ["var_r_mpos"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var_r_mpos"],
                  Some (M.get_var (| "memPtr" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 96])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 96]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      [Literal.number 96]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "var_p_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.call_function (|
                        "mload",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.get_var (| "var_p_mpos" |);
                              [Literal.number 32]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["addr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "addr" |);
                      [Literal.number 0x05]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_3"],
                  Some (M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "call",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.call_function (|
                                "gas",
                                []
                              |);
                              M.call_function (|
                                "not",
                                [
                                  [Literal.number 1999]
                                ]
                              |)
                            ]
                          |);
                          [Literal.number 7];
                          [Literal.number 0];
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 0x80];
                          M.get_var (| "memPtr" |);
                          [Literal.number 96]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.get_var (| "_3" |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "invalid",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_3"],
                  Some ([Literal.number 0])
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_mul_191",
            ["var_p_mpos"; "var_s"],
            ["var_r_153_mpos"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var_r_153_mpos"],
                  Some (M.get_var (| "memPtr" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 96])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      [Literal.number 96]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "calldatacopy",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.call_function (|
                        "calldatasize",
                        []
                      |);
                      [Literal.number 96]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "var_p_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.call_function (|
                        "mload",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.get_var (| "var_p_mpos" |);
                              [Literal.number 32]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["addr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "addr" |);
                      M.get_var (| "var_s" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_3"],
                  Some (M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "call",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.call_function (|
                                "gas",
                                []
                              |);
                              M.call_function (|
                                "not",
                                [
                                  [Literal.number 1999]
                                ]
                              |)
                            ]
                          |);
                          [Literal.number 7];
                          [Literal.number 0];
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 0x80];
                          M.get_var (| "memPtr" |);
                          [Literal.number 96]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.get_var (| "_3" |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "invalid",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_3"],
                  Some ([Literal.number 0])
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "checked_add_uint256_4013",
            ["x"],
            ["sum"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["sum"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "x" |);
                      [Literal.number 0x01]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      M.get_var (| "x" |);
                      M.get_var (| "sum" |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x11",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "checked_add_t_uint256",
            ["x"],
            ["sum"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["sum"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "x" |);
                      [Literal.number 0x02]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      M.get_var (| "x" |);
                      M.get_var (| "sum" |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x11",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "checked_add_uint256",
            ["x"],
            ["sum"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["sum"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "x" |);
                      [Literal.number 0x03]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      M.get_var (| "x" |);
                      M.get_var (| "sum" |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x11",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "checked_add_uint256_4036",
            ["x"],
            ["sum"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["sum"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "x" |);
                      [Literal.number 0x04]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      M.get_var (| "x" |);
                      M.get_var (| "sum" |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x11",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "checked_add_uint256_4039",
            ["x"],
            ["sum"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["sum"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "x" |);
                      [Literal.number 0x05]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      M.get_var (| "x" |);
                      M.get_var (| "sum" |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x11",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_verify",
            ["var_input_mpos"; "var_proof_mpos"],
            ["var"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["var"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1224_mpos"],
                  Some (M.call_function (|
                    "fun_verifyingKey",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr"],
                  Some (M.call_function (|
                    "checked_add_uint256_4013",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "var_input_mpos" |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "expr_1224_mpos" |);
                      [Literal.number 224]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "require_helper",
                    [
                      M.call_function (|
                        "eq",
                        [
                          M.get_var (| "expr" |);
                          M.call_function (|
                            "mload",
                            [
                              M.call_function (|
                                "mload",
                                [
                                  M.get_var (| "_1" |)
                                ]
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1246_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1246_mpos" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1246_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["var_vk_x_mpos"],
                  Some (M.get_var (| "expr_1246_mpos" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["var_i"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                do!
                  M.scope (
                    M.pure BlockUnit.Tt
                  ) in
                ltac:(M.monadic (
                  M.for_ (|
                    ltac:(M.monadic (
                      [Literal.number 0x01]
                    )),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["var_i"],
                          Some (M.call_function (|
                            "add",
                            [
                              M.get_var (| "var_i" |);
                              [Literal.number 0x01]
                            ]
                          |))
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    ),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.if_ (|
                          M.call_function (|
                            "iszero",
                            [
                              M.call_function (|
                                "lt",
                                [
                                  M.get_var (| "var_i" |);
                                  M.call_function (|
                                    "mload",
                                    [
                                      M.get_var (| "var_input_mpos" |)
                                    ]
                                  |)
                                ]
                              |)
                            ]
                          |),
                          M.scope (
                            do! ltac:(M.monadic (
                              M.break (||)
                            )) in
                            M.pure BlockUnit.Tt
                          )
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["_232_mpos"],
                          Some (M.call_function (|
                            "mload",
                            [
                              M.get_var (| "_1" |)
                            ]
                          |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["_234_mpos"],
                          Some (M.call_function (|
                            "mload",
                            [
                              M.call_function (|
                                "memory_array_index_access_uint256_dyn",
                                [
                                  M.get_var (| "_232_mpos" |);
                                  M.call_function (|
                                    "checked_add_uint256_4013",
                                    [
                                      M.get_var (| "var_i" |)
                                    ]
                                  |)
                                ]
                              |)
                            ]
                          |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["var_vk_x_mpos"],
                          Some (M.call_function (|
                            "fun_add",
                            [
                              M.get_var (| "var_vk_x_mpos" |);
                              M.call_function (|
                                "fun_mul_191",
                                [
                                  M.get_var (| "_234_mpos" |);
                                  M.call_function (|
                                    "mload",
                                    [
                                      M.call_function (|
                                        "memory_array_index_access_uint256_dyn",
                                        [
                                          M.get_var (| "var_input_mpos" |);
                                          M.get_var (| "var_i" |)
                                        ]
                                      |)
                                    ]
                                  |)
                                ]
                              |)
                            ]
                          |))
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    )
                  |)
                ))
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1287_mpos"],
                  Some (M.call_function (|
                    "fun_add",
                    [
                      M.get_var (| "var_vk_x_mpos" |);
                      M.call_function (|
                        "mload",
                        [
                          M.call_function (|
                            "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                            [
                              M.call_function (|
                                "mload",
                                [
                                  M.get_var (| "_1" |)
                                ]
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_247_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "var_proof_mpos" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_250_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "expr_1224_mpos" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1300_mpos"],
                  Some (M.call_function (|
                    "fun_negate",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.get_var (| "var_proof_mpos" |);
                              [Literal.number 32]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "cleanup_bool",
                    [
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "fun_pairingProd2",
                            [
                              M.get_var (| "_247_mpos" |);
                              M.get_var (| "_250_mpos" |);
                              M.get_var (| "expr_1300_mpos" |);
                              M.call_function (|
                                "fun_P2",
                                []
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["var"],
                        Some ([Literal.number 0x01])
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.leave (||)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_256_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1224_mpos" |);
                          [Literal.number 32]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_2"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "var_proof_mpos" |);
                      [Literal.number 64]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1319_mpos"],
                  Some (M.call_function (|
                    "fun_negate",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.get_var (| "var_proof_mpos" |);
                              [Literal.number 96]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "cleanup_bool",
                    [
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "fun_pairingProd2",
                            [
                              M.get_var (| "_256_mpos" |);
                              M.get_var (| "_mpos" |);
                              M.get_var (| "expr_1319_mpos" |);
                              M.call_function (|
                                "fun_P2",
                                []
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["var"],
                        Some ([Literal.number 0x02])
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.leave (||)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_3"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "var_proof_mpos" |);
                      [Literal.number 128]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_265_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_3" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_268_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1224_mpos" |);
                          [Literal.number 64]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1338_mpos"],
                  Some (M.call_function (|
                    "fun_negate",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.get_var (| "var_proof_mpos" |);
                              [Literal.number 160]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "cleanup_bool",
                    [
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "fun_pairingProd2",
                            [
                              M.get_var (| "_265_mpos" |);
                              M.get_var (| "_268_mpos" |);
                              M.get_var (| "expr_1338_mpos" |);
                              M.call_function (|
                                "fun_P2",
                                []
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["var"],
                        Some ([Literal.number 0x03])
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.leave (||)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_274_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "var_proof_mpos" |);
                          [Literal.number 192]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_277_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1224_mpos" |);
                          [Literal.number 96]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_281_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "var_proof_mpos" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1366_mpos"],
                  Some (M.call_function (|
                    "fun_negate",
                    [
                      M.call_function (|
                        "fun_add",
                        [
                          M.get_var (| "expr_1287_mpos" |);
                          M.call_function (|
                            "fun_add",
                            [
                              M.get_var (| "_281_mpos" |);
                              M.call_function (|
                                "mload",
                                [
                                  M.get_var (| "_3" |)
                                ]
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_287_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1224_mpos" |);
                          [Literal.number 160]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1373_mpos"],
                  Some (M.call_function (|
                    "fun_negate",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.get_var (| "expr_1224_mpos" |);
                              [Literal.number 128]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "cleanup_bool",
                    [
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "fun_pairingProd3",
                            [
                              M.get_var (| "_274_mpos" |);
                              M.get_var (| "_277_mpos" |);
                              M.get_var (| "expr_1366_mpos" |);
                              M.get_var (| "_287_mpos" |);
                              M.get_var (| "expr_1373_mpos" |);
                              M.call_function (|
                                "mload",
                                [
                                  M.get_var (| "_2" |)
                                ]
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["var"],
                        Some ([Literal.number 0x04])
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.leave (||)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1388_mpos"],
                  Some (M.call_function (|
                    "fun_add",
                    [
                      M.get_var (| "expr_1287_mpos" |);
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "var_proof_mpos" |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_300_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1395_mpos"],
                  Some (M.call_function (|
                    "fun_negate",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.call_function (|
                            "add",
                            [
                              M.get_var (| "var_proof_mpos" |);
                              [Literal.number 224]
                            ]
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_306_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1224_mpos" |);
                          [Literal.number 192]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1402_mpos"],
                  Some (M.call_function (|
                    "fun_negate",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "_3" |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "cleanup_bool",
                    [
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "fun_pairingProd3",
                            [
                              M.get_var (| "expr_1388_mpos" |);
                              M.get_var (| "_300_mpos" |);
                              M.get_var (| "expr_1395_mpos" |);
                              M.get_var (| "_306_mpos" |);
                              M.get_var (| "expr_1402_mpos" |);
                              M.call_function (|
                                "fun_P2",
                                []
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["var"],
                        Some ([Literal.number 0x05])
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.leave (||)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var"],
                  Some ([Literal.number 0])
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_P1",
            [],
            ["var_mpos"],
            M.scope (
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G1Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_24_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_24_mpos" |);
                      [Literal.number 0x01]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_24_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x02]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var_mpos"],
                  Some (M.get_var (| "expr_24_mpos" |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_negate",
            ["var_p_48_mpos"],
            ["var__mpos"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var__mpos"],
                  Some (M.get_var (| "memPtr" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr"],
                  Some (M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "var_p_48_mpos" |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.get_var (| "expr" |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["expr"],
                        Some (M.call_function (|
                          "iszero",
                          [
                            M.call_function (|
                              "mload",
                              [
                                M.call_function (|
                                  "add",
                                  [
                                    M.get_var (| "var_p_48_mpos" |);
                                    [Literal.number 32]
                                  ]
                                |)
                              ]
                            |)
                          ]
                        |))
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.get_var (| "expr" |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.declare (|
                        ["expr_70_mpos"],
                        Some (M.call_function (|
                          "allocate_memory_3951",
                          []
                        |))
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "mstore",
                          [
                            M.get_var (| "expr_70_mpos" |);
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "mstore",
                          [
                            M.call_function (|
                              "add",
                              [
                                M.get_var (| "expr_70_mpos" |);
                                [Literal.number 32]
                              ]
                            |);
                            [Literal.number 0]
                          ]
                        |)
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.assign (|
                        ["var__mpos"],
                        Some (M.get_var (| "expr_70_mpos" |))
                      |)
                    )) in
                    do! ltac:(M.monadic (
                      M.leave (||)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "var_p_48_mpos" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["diff"],
                  Some (M.call_function (|
                    "sub",
                    [
                      [Literal.number 0x30644e72e131a029b85045b68181585d97816a916871ca8d3c208c16d87cfd47];
                      M.call_function (|
                        "mod",
                        [
                          M.call_function (|
                            "mload",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.get_var (| "var_p_48_mpos" |);
                                  [Literal.number 32]
                                ]
                              |)
                            ]
                          |);
                          [Literal.number 0x30644e72e131a029b85045b68181585d97816a916871ca8d3c208c16d87cfd47]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "gt",
                    [
                      M.get_var (| "diff" |);
                      [Literal.number 0x30644e72e131a029b85045b68181585d97816a916871ca8d3c208c16d87cfd47]
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x11",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_83_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_83_mpos" |);
                      M.get_var (| "_1" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_83_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "diff" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var__mpos"],
                  Some (M.get_var (| "expr_83_mpos" |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_P2",
            [],
            ["var_32_mpos"],
            M.scope (
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G2Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_37_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_37_mpos" |);
                      [Literal.number 0x198e9393920d483a7260bfb731fb5d25f1aa493335a9e71297e485b7aef312c2]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_37_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x1800deef121f1e76426a00665e5c4479674322d4f75edadd46debd5cd992f6ed]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_40_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_40_mpos" |);
                      [Literal.number 0x090689d0585ff075ec9e99ad690c3395bc4b313370b38ef355acdadcd122975b]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_40_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x12c85ea5db8c6deb4aab71808dcb408fe3d1e7690c43d37b4ce6cc0166fa7daa]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_41_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_41_mpos" |);
                      M.get_var (| "expr_37_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_41_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_40_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var_32_mpos"],
                  Some (M.get_var (| "expr_41_mpos" |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "checked_mul_uint256",
            ["x"],
            ["product"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["product"],
                  Some (M.call_function (|
                    "mul",
                    [
                      M.get_var (| "x" |);
                      [Literal.number 0x06]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "or",
                        [
                          M.call_function (|
                            "iszero",
                            [
                              M.get_var (| "x" |)
                            ]
                          |);
                          M.call_function (|
                            "eq",
                            [
                              [Literal.number 0x06];
                              M.call_function (|
                                "div",
                                [
                                  M.get_var (| "product" |);
                                  M.get_var (| "x" |)
                                ]
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x11",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "memory_array_index_access_uint256",
            ["baseRef"],
            ["addr"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "baseRef" |);
                      [Literal.number 32]
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_pairing",
            ["var_p1_mpos"; "var_p2_mpos"],
            ["var"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "var_p1_mpos" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "require_helper",
                    [
                      M.call_function (|
                        "eq",
                        [
                          M.get_var (| "expr" |);
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "var_p2_mpos" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1"],
                  Some (M.call_function (|
                    "checked_mul_uint256",
                    [
                      M.get_var (| "expr" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_234_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_array_array_uint256_dyn_memory_ptr",
                    [
                      M.get_var (| "expr_1" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["var_i"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                do!
                  M.scope (
                    M.pure BlockUnit.Tt
                  ) in
                ltac:(M.monadic (
                  M.for_ (|
                    ltac:(M.monadic (
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "var_i" |);
                          M.get_var (| "expr" |)
                        ]
                      |)
                    )),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["var_i"],
                          Some (M.call_function (|
                            "add",
                            [
                              M.get_var (| "var_i" |);
                              [Literal.number 0x01]
                            ]
                          |))
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    ),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["_1"],
                          Some (M.call_function (|
                            "mload",
                            [
                              M.call_function (|
                                "mload",
                                [
                                  M.call_function (|
                                    "memory_array_index_access_uint256_dyn",
                                    [
                                      M.get_var (| "var_p1_mpos" |);
                                      M.get_var (| "var_i" |)
                                    ]
                                  |)
                                ]
                              |)
                            ]
                          |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["x"],
                          Some (M.call_function (|
                            "checked_mul_uint256",
                            [
                              M.get_var (| "var_i" |)
                            ]
                          |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["sum"],
                          Some ([Literal.number 0])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["y"],
                          Some ([Literal.number 0])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["sum"],
                          Some (M.get_var (| "x" |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["y"],
                          Some ([Literal.number 0x00])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "memory_array_index_access_uint256_dyn",
                                [
                                  M.get_var (| "expr_234_mpos" |);
                                  M.get_var (| "x" |)
                                ]
                              |);
                              M.get_var (| "_1" |)
                            ]
                          |)
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "memory_array_index_access_uint256_dyn",
                                [
                                  M.get_var (| "expr_234_mpos" |);
                                  M.call_function (|
                                    "checked_add_uint256_4013",
                                    [
                                      M.call_function (|
                                        "checked_mul_uint256",
                                        [
                                          M.get_var (| "var_i" |)
                                        ]
                                      |)
                                    ]
                                  |)
                                ]
                              |);
                              M.call_function (|
                                "mload",
                                [
                                  M.call_function (|
                                    "add",
                                    [
                                      M.call_function (|
                                        "mload",
                                        [
                                          M.call_function (|
                                            "memory_array_index_access_uint256_dyn",
                                            [
                                              M.get_var (| "var_p1_mpos" |);
                                              M.get_var (| "var_i" |)
                                            ]
                                          |)
                                        ]
                                      |);
                                      [Literal.number 32]
                                    ]
                                  |)
                                ]
                              |)
                            ]
                          |)
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["baseRef"],
                          Some (M.call_function (|
                            "mload",
                            [
                              M.call_function (|
                                "mload",
                                [
                                  M.call_function (|
                                    "memory_array_index_access_uint256_dyn",
                                    [
                                      M.get_var (| "var_p2_mpos" |);
                                      M.get_var (| "var_i" |)
                                    ]
                                  |)
                                ]
                              |)
                            ]
                          |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["addr"],
                          Some ([Literal.number 0x00])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["index"],
                          Some ([Literal.number 0x00])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["index"],
                          Some ([Literal.number 0x00])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["addr"],
                          Some (M.get_var (| "baseRef" |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "memory_array_index_access_uint256_dyn",
                                [
                                  M.get_var (| "expr_234_mpos" |);
                                  M.call_function (|
                                    "checked_add_t_uint256",
                                    [
                                      M.call_function (|
                                        "checked_mul_uint256",
                                        [
                                          M.get_var (| "var_i" |)
                                        ]
                                      |)
                                    ]
                                  |)
                                ]
                              |);
                              M.call_function (|
                                "mload",
                                [
                                  M.get_var (| "baseRef" |)
                                ]
                              |)
                            ]
                          |)
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "memory_array_index_access_uint256_dyn",
                                [
                                  M.get_var (| "expr_234_mpos" |);
                                  M.call_function (|
                                    "checked_add_uint256",
                                    [
                                      M.call_function (|
                                        "checked_mul_uint256",
                                        [
                                          M.get_var (| "var_i" |)
                                        ]
                                      |)
                                    ]
                                  |)
                                ]
                              |);
                              M.call_function (|
                                "mload",
                                [
                                  M.call_function (|
                                    "memory_array_index_access_uint256",
                                    [
                                      M.call_function (|
                                        "mload",
                                        [
                                          M.call_function (|
                                            "mload",
                                            [
                                              M.call_function (|
                                                "memory_array_index_access_uint256_dyn",
                                                [
                                                  M.get_var (| "var_p2_mpos" |);
                                                  M.get_var (| "var_i" |)
                                                ]
                                              |)
                                            ]
                                          |)
                                        ]
                                      |)
                                    ]
                                  |)
                                ]
                              |)
                            ]
                          |)
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["baseRef_1"],
                          Some (M.call_function (|
                            "mload",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.call_function (|
                                    "mload",
                                    [
                                      M.call_function (|
                                        "memory_array_index_access_uint256_dyn",
                                        [
                                          M.get_var (| "var_p2_mpos" |);
                                          M.get_var (| "var_i" |)
                                        ]
                                      |)
                                    ]
                                  |);
                                  [Literal.number 32]
                                ]
                              |)
                            ]
                          |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["addr_1"],
                          Some ([Literal.number 0x00])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.declare (|
                          ["index_1"],
                          Some ([Literal.number 0x00])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["index_1"],
                          Some ([Literal.number 0x00])
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["addr_1"],
                          Some (M.get_var (| "baseRef_1" |))
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "memory_array_index_access_uint256_dyn",
                                [
                                  M.get_var (| "expr_234_mpos" |);
                                  M.call_function (|
                                    "checked_add_uint256_4036",
                                    [
                                      M.call_function (|
                                        "checked_mul_uint256",
                                        [
                                          M.get_var (| "var_i" |)
                                        ]
                                      |)
                                    ]
                                  |)
                                ]
                              |);
                              M.call_function (|
                                "mload",
                                [
                                  M.get_var (| "baseRef_1" |)
                                ]
                              |)
                            ]
                          |)
                        |)
                      )) in
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "memory_array_index_access_uint256_dyn",
                                [
                                  M.get_var (| "expr_234_mpos" |);
                                  M.call_function (|
                                    "checked_add_uint256_4039",
                                    [
                                      M.call_function (|
                                        "checked_mul_uint256",
                                        [
                                          M.get_var (| "var_i" |)
                                        ]
                                      |)
                                    ]
                                  |)
                                ]
                              |);
                              M.call_function (|
                                "mload",
                                [
                                  M.call_function (|
                                    "memory_array_index_access_uint256",
                                    [
                                      M.call_function (|
                                        "mload",
                                        [
                                          M.call_function (|
                                            "add",
                                            [
                                              M.call_function (|
                                                "mload",
                                                [
                                                  M.call_function (|
                                                    "memory_array_index_access_uint256_dyn",
                                                    [
                                                      M.get_var (| "var_p2_mpos" |);
                                                      M.get_var (| "var_i" |)
                                                    ]
                                                  |)
                                                ]
                                              |);
                                              [Literal.number 32]
                                            ]
                                          |)
                                        ]
                                      |)
                                    ]
                                  |)
                                ]
                              |)
                            ]
                          |)
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    )
                  |)
                ))
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["zero_array_uint256_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_array_array_uint256",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["var_success"],
                  Some (M.call_function (|
                    "call",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.call_function (|
                            "gas",
                            []
                          |);
                          M.call_function (|
                            "not",
                            [
                              [Literal.number 1999]
                            ]
                          |)
                        ]
                      |);
                      [Literal.number 8];
                      [Literal.number 0];
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_234_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.call_function (|
                        "shl",
                        [
                          [Literal.number 0x05];
                          M.get_var (| "expr_1" |)
                        ]
                      |);
                      M.get_var (| "zero_array_uint256_mpos" |);
                      [Literal.number 32]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.get_var (| "var_success" |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "invalid",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "require_helper",
                    [
                      M.get_var (| "var_success" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["addr_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["index_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["index_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["addr_2"],
                  Some (M.get_var (| "zero_array_uint256_mpos" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var"],
                  Some (M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "iszero",
                        [
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "zero_array_uint256_mpos" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "allocate_and_zero_memory_struct_struct_VerifyingKey",
            [],
            ["memPtr"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.call_function (|
                    "allocate_memory_3958",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr" |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G2Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "memPtr_1" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 64]
                        ]
                      |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G2Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 96]
                        ]
                      |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G2Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_2"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr_2"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_2" |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_2" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 128]
                        ]
                      |);
                      M.get_var (| "memPtr_2" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 160]
                        ]
                      |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G2Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 192]
                        ]
                      |);
                      M.call_function (|
                        "allocate_and_zero_memory_struct_struct_G2Point",
                        []
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr" |);
                          [Literal.number 224]
                        ]
                      |);
                      [Literal.number 96]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_verifyingKey",
            [],
            ["var_vk_mpos"],
            M.scope (
              do! ltac:(M.monadic (
                M.assign (|
                  ["var_vk_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_struct_struct_VerifyingKey",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["size"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["_1"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["size"],
                  Some ([Literal.number 64])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_994_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_994_mpos" |);
                      [Literal.number 0x209dd15ebff5d46c4bd888e51a93cf99a7329636c63514396b4a452003a35bf7]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_994_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x04bf11ca01483bfa8b34b43561848d28905960114c8ac04049af4b6315a41678]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_997_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_997_mpos" |);
                      [Literal.number 0x2bb8324af6cfc93537a2ad1a445cfd0ca2a71acd7ac41fadbf933c2a51be344d]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_997_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x120a2a4cf30c1bf9845f20c6fe39e07ea2cce61f0c9bb048165fe5e4de877550]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_998_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_998_mpos" |);
                      M.get_var (| "expr_994_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_998_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_997_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "var_vk_mpos" |);
                      M.get_var (| "expr_998_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1008_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1008_mpos" |);
                      [Literal.number 0x2eca0c7238bf16e83e7a1e6c5d49540685ff51380f309842a98561558019fc02]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1008_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x03d3260361bb8451de5ff5ecd17f010ff22f5c31cdf184e9020b06fa5997db84]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "var_vk_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_1008_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1018_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1018_mpos" |);
                      [Literal.number 0x2e89718ad33c8bed92e210e81d1853435399a271913a6520736a4729cf0d51eb]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1018_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x01a9e2ffa2e92599b68e44de5bcf354fa2642bd4f26b259daa6f7ce3ed57aeb3]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1021_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1021_mpos" |);
                      [Literal.number 0x14a9a87b789a58af499b314e13c3d65bede56c07ea2d418d6874857b70763713]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1021_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x178fb49a2d6cd347dc58973ff49613a20757d0fcc22079f9abd10c3baee24590]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1022_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1022_mpos" |);
                      M.get_var (| "expr_1018_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1022_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_1021_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "var_vk_mpos" |);
                          M.get_var (| "size" |)
                        ]
                      |);
                      M.get_var (| "expr_1022_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1032_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1032_mpos" |);
                      [Literal.number 0x25f83c8b6ab9de74e7da488ef02645c5a16a6652c3c71a15dc37fe3a5dcb7cb1]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1032_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x22acdedd6308e3bb230d226d16a105295f523a8a02bfc5e8bd2da135ac4c245d]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1035_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1035_mpos" |);
                      [Literal.number 0x065bbad92e7c4e31bf3757f1fe7362a63fbfee50e7dc68da116e67d600d9bf68]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1035_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x06d302580dc0661002994e7cd3a7f224e7ddc27802777486bf80f40e4ca3cfdb]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1036_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1036_mpos" |);
                      M.get_var (| "expr_1032_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1036_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_1035_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "var_vk_mpos" |);
                          [Literal.number 96]
                        ]
                      |);
                      M.get_var (| "expr_1036_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1046_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1046_mpos" |);
                      [Literal.number 0x15794ab061441e51d01e94640b7e3084a07e02c78cf3103c542bc5b298669f21]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1046_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x14db745c6780e9df549864cec19c2daf4531f6ec0c89cc1c7436cc4d8d300c6d]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "var_vk_mpos" |);
                          [Literal.number 128]
                        ]
                      |);
                      M.get_var (| "expr_1046_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1056_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1056_mpos" |);
                      [Literal.number 0x1f39e4e4afc4bc74790a4a028aff2c3d2538731fb755edefd8cb48d6ea589b5e]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1056_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x283f150794b6736f670d6a1033f9b46c6f5204f50813eb85c8dc4b59db1c5d39]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1059_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1059_mpos" |);
                      [Literal.number 0x140d97ee4d2b36d99bc49974d18ecca3e7ad51011956051b464d9e27d46cc25e]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1059_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x0764bb98575bd466d32db7b15f582b2d5c452b36aa394b789366e5e3ca5aabd4]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1060_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1060_mpos" |);
                      M.get_var (| "expr_1056_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1060_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_1059_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "var_vk_mpos" |);
                          [Literal.number 160]
                        ]
                      |);
                      M.get_var (| "expr_1060_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1070_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1070_mpos" |);
                      [Literal.number 0x217cee0a9ad79a4493b5253e2e4e3a39fc2df38419f230d341f60cb064a0ac29]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1070_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x0a3d76f140db8418ba512272381446eb73958670f00cf46f1d9e64cba057b53c]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1073_mpos"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.get_var (| "size" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1073_mpos" |);
                      [Literal.number 0x26f64a8ec70387a13e41430ed3ee4a7db2059cc5fc13c067194bcc0cb49a9855]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1073_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x2fd72bd9edb657346127da132e5b82ab908f5816c826acb499e22f2412d1a2d7]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1074_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1074_mpos" |);
                      M.get_var (| "expr_1070_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1074_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "expr_1073_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "var_vk_mpos" |);
                          [Literal.number 192]
                        ]
                      |);
                      M.get_var (| "expr_1074_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1085_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_array_array_struct_G1Point_memory_ptr_dyn_memory_ptr",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_2"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.get_var (| "var_vk_mpos" |);
                      [Literal.number 224]
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "_2" |);
                      M.get_var (| "expr_1085_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1097_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1097_mpos" |);
                      [Literal.number 0x0aee46a7ea6e80a3675026dfa84019deee2a2dedb1bbe11d7fe124cb3efb4b5a]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1097_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x044747b6e9176e13ede3a4dfd0d33ccca6321b9acd23bf3683a60adc0366ebaf]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_456_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "_456_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_1097_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "_456_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1109_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1109_mpos" |);
                      [Literal.number 0x1e39e9f0f91fa7ff8047ffd90de08785777fe61c0e3434e728fce4cf35047ddc]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1109_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x2e0b64d75ebfa86d7f8f8e08abbe2e7ae6e0a1c0b34d028f19fa56e9450527cb]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_462_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "_462_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_1109_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "_462_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1121_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1121_mpos" |);
                      [Literal.number 0x1c36e713d4d54e3a9644dffca1fc524be4868f66572516025a61ca542539d43f]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1121_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x042dcc4525b82dfb242b09cb21909d5c22643dcdbe98c4d082cc2877e96b24db]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_468_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3972",
                        [
                          M.get_var (| "_468_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_1121_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3972",
                        [
                          M.get_var (| "_468_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_mpos" |);
                      [Literal.number 0x17d5d09b4146424bff7e6fb01487c477bbfcd0cdbbc92d5d6457aae0b6717cc5]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x02b5636903efbf46db9235bbe74045d21c138897fda32e079040db1a16c1a7a1]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_474_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3973",
                        [
                          M.get_var (| "_474_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3973",
                        [
                          M.get_var (| "_474_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1145_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1145_mpos" |);
                      [Literal.number 0x0f103f14a584d4203c27c26155b2c955f8dfa816980b24ba824e1972d6486a5d]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1145_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x0c4165133b9f5be17c804203af781bcf168da7386620479f9b885ecbcd27b17b]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_480_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_array_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "_480_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_1145_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_array_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "_480_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1157_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1157_mpos" |);
                      [Literal.number 0x232063b584fb76c8d07995bee3a38fa7565405f3549c6a918ddaa90ab971e7f8]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1157_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x2ac9b135a81d96425c92d02296322ad56ffb16299633233e4880f95aafa7fda7]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_486_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "_486_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_1157_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "_486_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1169_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1169_mpos" |);
                      [Literal.number 0x09b54f111d3b2d1b2fe1ae9669b3db3d7bf93b70f00647e65c849275de6dc7fe]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1169_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x18b2e77c63a3e400d6d1f1fbc6e1a1167bbca603d34d03edea231eb0ab7b14b4]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_492_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn",
                        [
                          M.get_var (| "_492_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_1169_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn",
                        [
                          M.get_var (| "_492_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1181_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1181_mpos" |);
                      [Literal.number 0x0c54b42137b67cc268cbb53ac62b00ecead23984092b494a88befe58445a244a]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1181_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x18e3723d37fae9262d58b548a0575f59d9c3266db7afb4d5739555837f6b8b3e]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_498_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn",
                        [
                          M.get_var (| "_498_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_1181_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn",
                        [
                          M.get_var (| "_498_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1193_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1193_mpos" |);
                      [Literal.number 0x0a6de0e2240aa253f46ce0da883b61976e3588146e01c9d8976548c145fe6e4a]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1193_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x04fbaa3a4aed4bb77f30ebb07a3ec1c7d77a7f2edd75636babfeff97b1ea686e]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_504_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_array_uint256_dyn",
                        [
                          M.get_var (| "_504_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_1193_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_array_uint256_dyn",
                        [
                          M.get_var (| "_504_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_1205_mpos"],
                  Some (M.call_function (|
                    "allocate_memory_3951",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "expr_1205_mpos" |);
                      [Literal.number 0x111e2e2a5f8828f80ddad08f9f74db56dac1cc16c1cb278036f79a84cf7a116f]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_1205_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      [Literal.number 0x1d7d62e192b219b9808faa906c5ced871788f6339e8d91b83ac1343e20a16b30]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_510_mpos"],
                  Some (M.call_function (|
                    "mload",
                    [
                      M.get_var (| "_2" |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_4089",
                        [
                          M.get_var (| "_510_mpos" |)
                        ]
                      |);
                      M.get_var (| "expr_1205_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_4089",
                        [
                          M.get_var (| "_510_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_pairingProd2",
            ["var_a1_359_mpos"; "var_a2_mpos"; "var_b1_mpos"; "var_b2_mpos"],
            ["var"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_383_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_array_array_struct_G1Point_dyn",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_395_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_array_array_struct_G2Point_dyn",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "expr_383_mpos" |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_383_mpos" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "var_a1_359_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "expr_383_mpos" |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          [Literal.number 0x01];
                          M.call_function (|
                            "mload",
                            [
                              M.get_var (| "expr_383_mpos" |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "expr_383_mpos" |);
                          [Literal.number 64]
                        ]
                      |);
                      M.get_var (| "var_b1_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_383_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_395_mpos" |)
                        ]
                      |);
                      M.get_var (| "var_a2_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_395_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_395_mpos" |)
                        ]
                      |);
                      M.get_var (| "var_b2_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_395_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var"],
                  Some (M.call_function (|
                    "fun_pairing",
                    [
                      M.get_var (| "expr_383_mpos" |);
                      M.get_var (| "expr_395_mpos" |)
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          );
          Code.Function.make (
            "fun_pairingProd3",
            ["var_a1_mpos"; "var_a2_433_mpos"; "var_b1_436_mpos"; "var_b2_439_mpos"; "var_c1_mpos"; "var_c2_mpos"],
            ["var"],
            M.scope (
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["memPtr_1"],
                  Some (M.call_function (|
                    "allocate_memory",
                    [
                      M.call_function (|
                        "array_allocation_size_array_uint256_dyn",
                        [
                          [Literal.number 0x03]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.get_var (| "memPtr_1" |);
                      [Literal.number 0x03]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["memPtr"],
                  Some (M.get_var (| "memPtr_1" |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["_1"],
                  Some (M.call_function (|
                    "add",
                    [
                      M.call_function (|
                        "array_allocation_size_array_uint256_dyn",
                        [
                          [Literal.number 0x03]
                        ]
                      |);
                      M.call_function (|
                        "not",
                        [
                          [Literal.number 31]
                        ]
                      |)
                    ]
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["i"],
                  Some ([Literal.number 0])
                |)
              )) in
              do! ltac:(M.monadic (
                do!
                  M.scope (
                    M.pure BlockUnit.Tt
                  ) in
                ltac:(M.monadic (
                  M.for_ (|
                    ltac:(M.monadic (
                      M.call_function (|
                        "lt",
                        [
                          M.get_var (| "i" |);
                          M.get_var (| "_1" |)
                        ]
                      |)
                    )),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.assign (|
                          ["i"],
                          Some (M.call_function (|
                            "add",
                            [
                              M.get_var (| "i" |);
                              [Literal.number 32]
                            ]
                          |))
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    ),
                    M.scope (
                      do! ltac:(M.monadic (
                        M.expr_stmt (|
                          M.call_function (|
                            "mstore",
                            [
                              M.call_function (|
                                "add",
                                [
                                  M.call_function (|
                                    "add",
                                    [
                                      M.get_var (| "memPtr_1" |);
                                      M.get_var (| "i" |)
                                    ]
                                  |);
                                  [Literal.number 32]
                                ]
                              |);
                              M.call_function (|
                                "allocate_and_zero_memory_struct_struct_G1Point",
                                []
                              |)
                            ]
                          |)
                        |)
                      )) in
                      M.pure BlockUnit.Tt
                    )
                  |)
                ))
              )) in
              do! ltac:(M.monadic (
                M.declare (|
                  ["expr_472_mpos"],
                  Some (M.call_function (|
                    "allocate_and_zero_memory_array_array_struct_G2Point_memory_ptr_dyn_memory_ptr",
                    []
                  |))
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "memPtr_1" |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "add",
                        [
                          M.get_var (| "memPtr_1" |);
                          [Literal.number 32]
                        ]
                      |);
                      M.get_var (| "var_a1_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "mload",
                        [
                          M.get_var (| "memPtr_1" |)
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.expr_stmt (|
                        M.call_function (|
                          "panic_error_0x32",
                          []
                        |)
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "memPtr_1" |)
                        ]
                      |);
                      M.get_var (| "var_b1_436_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "memPtr_1" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3972",
                        [
                          M.get_var (| "memPtr_1" |)
                        ]
                      |);
                      M.get_var (| "var_c1_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3972",
                        [
                          M.get_var (| "memPtr_1" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_472_mpos" |)
                        ]
                      |);
                      M.get_var (| "var_a2_433_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_array_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_472_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_472_mpos" |)
                        ]
                      |);
                      M.get_var (| "var_b2_439_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_t_uint256_dyn_memory_ptr",
                        [
                          M.get_var (| "expr_472_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3972",
                        [
                          M.get_var (| "expr_472_mpos" |)
                        ]
                      |);
                      M.get_var (| "var_c2_mpos" |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "pop",
                    [
                      M.call_function (|
                        "memory_array_index_access_uint256_dyn_3972",
                        [
                          M.get_var (| "expr_472_mpos" |)
                        ]
                      |)
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.assign (|
                  ["var"],
                  Some (M.call_function (|
                    "fun_pairing",
                    [
                      M.get_var (| "memPtr_1" |);
                      M.get_var (| "expr_472_mpos" |)
                    ]
                  |))
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          )
        ];
      Code.body :=
        M.scope (
          do! ltac:(M.monadic (
            M.scope (
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "mstore",
                    [
                      [Literal.number 64];
                      [Literal.number 128]
                    ]
                  |)
                |)
              )) in
              do! ltac:(M.monadic (
                M.if_ (|
                  M.call_function (|
                    "iszero",
                    [
                      M.call_function (|
                        "lt",
                        [
                          M.call_function (|
                            "calldatasize",
                            []
                          |);
                          [Literal.number 4]
                        ]
                      |)
                    ]
                  |),
                  M.scope (
                    do! ltac:(M.monadic (
                      M.switch (|
                        M.call_function (|
                          "shr",
                          [
                            [Literal.number 224];
                            M.call_function (|
                              "calldataload",
                              [
                                [Literal.number 0]
                              ]
                            |)
                          ]
                        |),
                        [
                          (
                            Some (Literal.number 0x26121ff0),
                            M.scope (
                              do! ltac:(M.monadic (
                                M.if_ (|
                                  M.call_function (|
                                    "callvalue",
                                    []
                                  |),
                                  M.scope (
                                    do! ltac:(M.monadic (
                                      M.expr_stmt (|
                                        M.call_function (|
                                          "revert",
                                          [
                                            [Literal.number 0];
                                            [Literal.number 0]
                                          ]
                                        |)
                                      |)
                                    )) in
                                    M.pure BlockUnit.Tt
                                  )
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.if_ (|
                                  M.call_function (|
                                    "slt",
                                    [
                                      M.call_function (|
                                        "add",
                                        [
                                          M.call_function (|
                                            "calldatasize",
                                            []
                                          |);
                                          M.call_function (|
                                            "not",
                                            [
                                              [Literal.number 3]
                                            ]
                                          |)
                                        ]
                                      |);
                                      [Literal.number 0]
                                    ]
                                  |),
                                  M.scope (
                                    do! ltac:(M.monadic (
                                      M.expr_stmt (|
                                        M.call_function (|
                                          "revert",
                                          [
                                            [Literal.number 0];
                                            [Literal.number 0]
                                          ]
                                        |)
                                      |)
                                    )) in
                                    M.pure BlockUnit.Tt
                                  )
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.declare (|
                                  ["zero_t_struct_G1Point_mpos"],
                                  Some (M.call_function (|
                                    "allocate_and_zero_memory_struct_struct_G1Point",
                                    []
                                  |))
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.declare (|
                                  ["zero_struct_G1Point_mpos"],
                                  Some (M.call_function (|
                                    "allocate_and_zero_memory_struct_struct_G1Point",
                                    []
                                  |))
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "mstore",
                                    [
                                      M.get_var (| "zero_t_struct_G1Point_mpos" |);
                                      [Literal.number 0x01]
                                    ]
                                  |)
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "mstore",
                                    [
                                      M.call_function (|
                                        "add",
                                        [
                                          M.get_var (| "zero_t_struct_G1Point_mpos" |);
                                          [Literal.number 32]
                                        ]
                                      |);
                                      [Literal.number 0x02]
                                    ]
                                  |)
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "mstore",
                                    [
                                      M.get_var (| "zero_struct_G1Point_mpos" |);
                                      [Literal.number 0x01]
                                    ]
                                  |)
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "mstore",
                                    [
                                      M.call_function (|
                                        "add",
                                        [
                                          M.get_var (| "zero_struct_G1Point_mpos" |);
                                          [Literal.number 32]
                                        ]
                                      |);
                                      [Literal.number 0x02]
                                    ]
                                  |)
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.declare (|
                                  ["expr_mpos"],
                                  Some (M.call_function (|
                                    "fun_add",
                                    [
                                      M.get_var (| "zero_t_struct_G1Point_mpos" |);
                                      M.get_var (| "zero_struct_G1Point_mpos" |)
                                    ]
                                  |))
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.declare (|
                                  ["expr_mpos_1"],
                                  Some (M.call_function (|
                                    "fun_mul_3948",
                                    [
                                      M.get_var (| "zero_t_struct_G1Point_mpos" |)
                                    ]
                                  |))
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.declare (|
                                  ["_1"],
                                  Some (M.call_function (|
                                    "mload",
                                    [
                                      M.get_var (| "expr_mpos" |)
                                    ]
                                  |))
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.declare (|
                                  ["expr"],
                                  Some (M.call_function (|
                                    "eq",
                                    [
                                      M.get_var (| "_1" |);
                                      M.call_function (|
                                        "mload",
                                        [
                                          M.get_var (| "expr_mpos_1" |)
                                        ]
                                      |)
                                    ]
                                  |))
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.if_ (|
                                  M.get_var (| "expr" |),
                                  M.scope (
                                    do! ltac:(M.monadic (
                                      M.declare (|
                                        ["_2"],
                                        Some (M.call_function (|
                                          "mload",
                                          [
                                            M.call_function (|
                                              "add",
                                              [
                                                M.get_var (| "expr_mpos" |);
                                                [Literal.number 32]
                                              ]
                                            |)
                                          ]
                                        |))
                                      |)
                                    )) in
                                    do! ltac:(M.monadic (
                                      M.assign (|
                                        ["expr"],
                                        Some (M.call_function (|
                                          "eq",
                                          [
                                            M.get_var (| "_2" |);
                                            M.call_function (|
                                              "mload",
                                              [
                                                M.call_function (|
                                                  "add",
                                                  [
                                                    M.get_var (| "expr_mpos_1" |);
                                                    [Literal.number 32]
                                                  ]
                                                |)
                                              ]
                                            |)
                                          ]
                                        |))
                                      |)
                                    )) in
                                    M.pure BlockUnit.Tt
                                  )
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.declare (|
                                  ["memPos"],
                                  Some (M.call_function (|
                                    "mload",
                                    [
                                      [Literal.number 64]
                                    ]
                                  |))
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "mstore",
                                    [
                                      M.get_var (| "memPos" |);
                                      M.call_function (|
                                        "iszero",
                                        [
                                          M.call_function (|
                                            "iszero",
                                            [
                                              M.get_var (| "expr" |)
                                            ]
                                          |)
                                        ]
                                      |)
                                    ]
                                  |)
                                |)
                              )) in
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "return",
                                    [
                                      M.get_var (| "memPos" |);
                                      [Literal.number 32]
                                    ]
                                  |)
                                |)
                              )) in
                              M.pure BlockUnit.Tt
                            )
                          );
                          (
                            Some (Literal.number 0x6dae022f),
                            M.scope (
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "external_fun_verifyTx",
                                    []
                                  |)
                                |)
                              )) in
                              M.pure BlockUnit.Tt
                            )
                          );
                          (
                            Some (Literal.number 0xa8aa1b31),
                            M.scope (
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "external_fun_pair",
                                    []
                                  |)
                                |)
                              )) in
                              M.pure BlockUnit.Tt
                            )
                          );
                          (
                            Some (Literal.number 0xe10fb98f),
                            M.scope (
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "external_fun_testMul",
                                    []
                                  |)
                                |)
                              )) in
                              M.pure BlockUnit.Tt
                            )
                          );
                          (
                            Some (Literal.number 0xe2179b8e),
                            M.scope (
                              do! ltac:(M.monadic (
                                M.expr_stmt (|
                                  M.call_function (|
                                    "external_fun_g",
                                    []
                                  |)
                                |)
                              )) in
                              M.pure BlockUnit.Tt
                            )
                          )
                        ]
                      |)
                    )) in
                    M.pure BlockUnit.Tt
                  )
                |)
              )) in
              do! ltac:(M.monadic (
                M.expr_stmt (|
                  M.call_function (|
                    "revert",
                    [
                      [Literal.number 0];
                      [Literal.number 0]
                    ]
                  |)
                |)
              )) in
              M.pure BlockUnit.Tt
            )
          )) in
          M.pure BlockUnit.Tt
        );
    |}.
  End deployed.
End Test.

Import Ltac2.

Definition codes : list Code.t :=
  ltac2:(
    let codes := Code.get_codes () in
    exact $codes
  ).
